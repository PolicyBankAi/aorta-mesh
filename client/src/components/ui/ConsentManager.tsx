...
export const ConsentManager: React.FC<ConsentManagerProps> = ({ userId }) => {
  ...
  if (loading) {
    return (
      <div className="p-4 border rounded-lg shadow bg-gray-50 dark:bg-gray-800">
  ...
  return (
    <div className="p-4 border rounded-lg shadow bg-white dark:bg-gray-900">
      ...
      <button
        ...
        onClick={() => giveConsent("gdpr_data_processing")}
      >
        Accept GDPR Consent
      </button>
      ...
      <ul className="mt-4 space-y-2">
        {consents.length === 0 ? (
          ...
          consents.map((c) => (
            <li
              key={c.id}
              className="flex justify-between items-center p-2 border rounded bg-gray-50 dark:bg-gray-800"
            >
              <span>
              ...
              {!c.withdrawn && (
                <button
                  className="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-sm transition"
