import { BarChart3, FileText, ClipboardList, Microscope, Link2, FolderOpen, Plug, User, Settings, Users, Shield, Archive } from "lucide-react";
...
export default function Sidebar() {
    ...
    return (<div className="hidden lg:flex lg:w-64 lg:flex-col" data-testid="sidebar">
      <div className="flex flex-col flex-grow pt-5 pb-4 overflow-y-auto bg-black border-r border-cyan-400">
        ...
        <nav className="mt-8 flex-1 px-2 space-y-1" data-testid="sidebar-nav">
          {navigation.map((item) => {
            ...
            return (<RouterLink key={item.name} href={item.href} className={`${isActive
                    ? 'bg-cyan-600 text-black border border-cyan-400'
                    : 'text-cyan-300 hover:bg-gray-900 hover:text-cyan-400 border border-transparent'} group flex items-center px-3 py-2 text-sm font-medium rounded-md`} data-testid={`link-${item.name.toLowerCase().replace(/\s+/g, '-')}`}>
                <Icon className={`${isActive ? 'text-black' : 'text-cyan-400 group-hover:text-cyan-300'} mr-3 flex-shrink-0 h-4 w-4`}/>
                {item.name}
              </RouterLink>);
        })}
        </nav>
